...
RSpec.shared_examples 'Wallet Owner Examples' do
  ...
  describe '#deposit' do
    context 'with valid amount' do
      ...
      it 'adds balance' do
        current_balance = subject.wallet.balance
        expect { subject.deposit(amount) }.to change { subject.wallet.balance }.from(current_balance).to(current_balance + amount)
      end
    end
    ...
    context 'with negative amount' do
      ...
      it 'keeps balance' do
        expect { subject.deposit(amount) }.not_to change { subject.wallet.balance }
      end
    end
  end
  ...
  describe '#withdraw' do
    context 'with valid amount' do
      let(:amount) { subject.wallet.balance }
      ...
      context 'and balance sufficient' do
        ...
        it 'reduces balance' do
          current_balance = subject.wallet.balance
          expect { subject.withdraw(amount) }.to change { subject.wallet.balance }.from(current_balance).to(current_balance - amount)
        end
      end
      ...
      context 'and balance insufficient' do
        ...
        it 'keeps balance' do
          expect { subject.withdraw(amount) }.not_to change { subject.wallet.balance }
        end
      end
    end
